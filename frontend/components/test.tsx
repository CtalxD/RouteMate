...
//     // Disable map interaction when menu is open
//     setIsMapInteractionEnabled(!menuVisible);
//   };
...
//   // Improved map URL generation with better controls
...
//     // Create marker parameters for start and end points
//     const startMarker = `${selectedFromLocation?.lat},${selectedFromLocation?.lng}`;
//     const endMarker = `${selectedToLocation?.lat},${selectedToLocation?.lng}`;
...
//     // Create a more stable URL with fixed center and zoom
//     return `https://www.openstreetmap.org/export/embed.html?bbox=${bounds}&layer=mapnik&marker=${startMarker}&marker=${endMarker}&center=${centerLat},${centerLng}&zoom=${zoomLevel}`;
//   };
...
//       return `https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&layer=mapnik&marker=${userLocation.lat},${userLocation.lng}&center=${userLocation.lat},${userLocation.lng}&zoom=${zoomLevel}`;
//     } else {
...
//       return `https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&layer=mapnik&marker=${defaultLocation.lat},${defaultLocation.lng}&center=${defaultLocation.lat},${defaultLocation.lng}&zoom=${zoomLevel}`;
//     }
//   };
...
//         try {
//           // Add message listener to receive updates from the map iframe
//           window.addEventListener('message', (event) => {
//             // Check if the message is from our map
//             if (event.data && event.data.type === 'map_update') {
//               // Update our state based on map changes
//               if (event.data.zoom) {
//                 setZoomLevel(event.data.zoom);
//               }
//               if (event.data.center) {
//                 // Handle center updates if needed
//               }
//             }
//           });
...
//   // Add script to iframe to improve touch handling
//   const injectMapControlScript = (iframe: HTMLIFrameElement | null) => {
//     if (!iframe) return;
...
//       if (iframeDoc.querySelector('#map-control-script')) return;
      
//       const script = iframeDoc.createElement('script');
//       script.id = 'map-control-script';
//       script.textContent = `
...
//             var hammertime = new Hammer(map);
//             hammertime.get('pan').set({ direction: Hammer.DIRECTION_ALL });
            
//             // Disable default touch behaviors that cause flickering
...
//             // Send map updates to parent
//             window.addEventListener('zoomend', function() {
//               var zoom = map._zoom;
...
//       // Add hammer.js for better touch handling if not already present
//       if (!iframeDoc.querySelector('#hammer-script')) {
//         const hammerScript = iframeDoc.createElement('script');
//         hammerScript.id = 'hammer-script';
//         hammerScript.src = 'https://cdnjs.cloudflare.com/ajax/libs/hammer.js/2.0.8/hammer.min.js';
//         iframeDoc.head.appendChild(hammerScript);
//       }
...
//         }
//         /* Increase size of UI controls for better touch */
...
//     } catch (error) {
//       console.error("Error injecting map control script:", error);
//     }
//   };
...
//   // Handle iframe onLoad to inject customizations
//   const handleIframeLoad = () => {
//     if (mapIframeRef.current) {
//       injectMapControlScript(mapIframeRef.current);
//     }
//   };
...
//             style={{
//               ...styles.map,
//               pointerEvents: isMapInteractionEnabled ? "auto" : "none",
//             }}
//             title="OpenStreetMap"
//             allow="geolocation"
...
//             scrolling="no"
//           />
//           {/* Add transparent overlay to prevent accidental scrolling */}
//           {!isMapInteractionEnabled && <View style={styles.mapOverlay} />}
//         </View>
//       )}
